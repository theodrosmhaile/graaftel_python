...
#         Scores, Students, Items, dat, nSkills
    ...
    def load_model(self, model, data, studentMode=False, nSkills=4, decayingAlpha = True,baseAlpha=0, studentAlpha=None, alpha = None, nEpochs=None):
       ...
       # M = m_import#['model']
        ...
        ### There might be unregistered students and items in the dataset

        Registered_students  = set([self.students[i].name for i in self.students ])
        Registered_items  = set([self.items[i].name for i in self.items ])

        New_students = set([str(self.data[0][s]) for s in range(len(self.data))])
        New_items = set([str(self.data[1][s]) for s in range(len(self.data))])

        items_to_Register = list(New_items.difference(Registered_items))
        students_to_Register = list(New_students.difference(Registered_students))

        if len(items_to_Register)> 0:
            print("Registering new items...")

            for x in items_to_Register:
                self.items[x] = Item(name=x, nSkills =nSkills)

        if len(students_to_Register)> 0:
            print("Registering new students... ")

            for x in students_to_Register:
                self.students[x] = Student(name=x, nSkills=nSkills)
        #####################################################################
...
def calculateError(model, student=None): ## This function computes the final error after model runs
    ...
    instances = model.scores if student is None else [i for i in model.scores if model.scores[i].student== student]
